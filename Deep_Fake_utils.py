...
install("pytorchcv-0.0.55-py2.py3-none-any.whl")
install("pytube")
install("moviepy")
install("youtube-dl")
install("pafy")
...
#######################################################################
def loadResNextModel(modelWeightsPath):
  ...
  model_ft.load_state_dict(torch.load(modelWeightsPath, map_location=device))
  model_ft.to(device)
  model_ft.eval()
  return model_ft
  
def loadXceptionModel(modelWeightsPath):
  ...
  modelXception.load_state_dict(torch.load(modelWeightsPath,map_location=device))
  modelXception.eval()
...
def DeepFake_Inference(video,modelXception,modelResNeXt,threshold=0.3,r1=0.224,r2=0.6124):
...
def Inference_on_video(output_file_path, video_file_path, modelXception,modelResNeXt, threshold=0.3,r1=0.224,r2=0.6124):
    '''
    ...
    class_prediction = DeepFake_Inference(video_file_path, modelXception,modelResNeXt, threshold=0.3,r1=0.224,r2=0.6124)
